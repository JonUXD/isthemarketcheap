...
async function fetchYahooPrice(symbol) {
  try {
    // Adding timestamp to URL to bypass stale caches on Vercel/proxies
    const ts = Date.now();
    const url = `https://query1.finance.yahoo.com/v8/finance/chart/${symbol}?interval=1d&range=max&ts=${ts}`;
    ...
    // Use adjclose for better ATH tracking (includes dividends/splits)
    // Yahoo sometimes puts it in adjclose[0].adjclose
    const adjCloseArr = indicators.adjclose && indicators.adjclose[0].adjclose;
    const prices = adjCloseArr || indicators.quote[0].close;

    const timestamps = result.timestamp;

    // Find historical ATH and its date
    let ath = -1;
    let athIndex = -1;
    ...
    // FIX: If current price is higher than historical ATH, sync them
    // This handles the "stale max range" issue where Yahoo hasn't updated its history array yet
    if (currentPrice >= ath) {
      ath = currentPrice;
